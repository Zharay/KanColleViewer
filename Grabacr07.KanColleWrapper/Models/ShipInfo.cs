...
namespace Grabacr07.KanColleWrapper.Models
{
	...
	public class ShipInfo : RawDataWrapper<kcsapi_mst_ship>, IIdentifiable
	{
		...
		/// 艦の名称を取得します。
		/// </summary>
		public string Name => KanColleClient.Current.Translations.GetTranslation(RawData.api_name, TranslationType.Ships, this.RawData);

		/// <summary>
		/// Get the untranslated version of the name. If it is untranslated, return nothing.
		/// </summary>
		public string UntranslatedName => this.Name != RawData.api_name ? RawData.api_name : "";
		...
		/// 回避の最大値を取得します。
		/// </summary>
		public int MaxEvasion => this.RawData.api_kaih.Get(1) ?? 0;

	    /// <summary>
		/// 対潜の最大値を取得します (ASW: Anti-submarine warfare)。
		/// </summary>
		public int MaxASW => this.RawData.api_tais.Get(1) ?? 0;

	    /// <summary>
		/// 索敵の最大値を取得します。
		/// </summary>
		public int MaxLOS => this.RawData.api_saku.Get(1) ?? 0;
		*/
